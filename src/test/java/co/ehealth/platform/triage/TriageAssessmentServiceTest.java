...
class TriageAssessmentServiceTest {
    ...
    @Test
    void captures_additional_triage_fields_when_provided() {
        TriageAssessmentRepository repository = mock(TriageAssessmentRepository.class);
        VisitService visits = mock(VisitService.class);
        UUID visitId = UUID.randomUUID();
        UUID patientId = UUID.randomUUID();
        ...
        service.capture(command(visitId, 120, 80, 80, "37.0", 16, false), UUID.randomUUID(),
                "Independent with walking aid", "Protein trace present");

        verify(repository).save(org.mockito.ArgumentMatchers.argThat(assessment ->
                "Independent with walking aid".equals(assessment.getMobilityAssessment()) &&
                "Protein trace present".equals(assessment.getUrineTestFindings())));
    }
