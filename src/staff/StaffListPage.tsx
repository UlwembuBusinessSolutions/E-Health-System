...
import { Check, Copy, KeyRound, LockKeyholeOpen, Plus, Search } from "lucide-react";
import { listStaff, resetStaffPassword, setStaffEnabled, unlockStaffAccount } from "@/shared/api/staff";
...
export function StaffListPage() {
  ...
  const unlockAccount = useMutation({
    mutationFn: (id: string) => unlockStaffAccount(id),
    onMutate: () => setActionError(null),
    onSuccess: () => queryClient.invalidateQueries({ queryKey: ["staff", "list"] }),
    onError: (error) => {
                        ...
                        <div className="flex items-center justify-end gap-2">
                          ...
                          {s.status === "LOCKED" && (
                            <Button
                              ...
                              loading={unlockAccount.isPending && unlockAccount.variables === s.id}
                              onClick={() => unlockAccount.mutate(s.id)}
                            >
                              Unlock
                            </Button>
                          )}
