...
//         // Exactly one of idNumber/passportNumber is required — enforced here,
//         // not just left to the DB's own CHECK constraint, so this fails with a
//         // clean validation message rather than a raw DataIntegrityViolationException.
...
//         List<Patient> results = patientRepository.search(trimmed, dob);
...
public class PatientService {
    ...
    public List<Patient> search(String query, LocalDate dob) {
        ...
        return patientRepository.search(trimmed, dob);
    }
