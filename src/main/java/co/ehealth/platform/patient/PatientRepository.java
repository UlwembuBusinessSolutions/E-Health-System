...
public interface PatientRepository extends JpaRepository<Patient, UUID> {
    ...
    @Query("SELECT DISTINCT p FROM Patient p LEFT JOIN FETCH p.nextOfKin WHERE p.id = :id")
    java.util.Optional<Patient> findWithNextOfKinById(@Param("id") UUID id);
    ...
    @Query("SELECT DISTINCT p FROM Patient p LEFT JOIN FETCH p.nextOfKin WHERE "
            + "LOWER(p.firstName) LIKE LOWER(CONCAT('%', :query, '%')) "
            + "OR LOWER(p.lastName) LIKE LOWER(CONCAT('%', :query, '%')) "
            + "OR LOWER(p.mpiNumber) = LOWER(:query) OR LOWER(p.idNumber) = LOWER(:query) "
