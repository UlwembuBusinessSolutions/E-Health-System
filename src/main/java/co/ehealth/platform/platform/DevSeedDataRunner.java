...
// hand-provisions a tenant through the platform console first. This gives
// a new developer one ready-made, populated tenant to actually click around
...
public class DevSeedDataRunner implements ApplicationRunner {
    ...
    public void run(ApplicationArguments args) {
        if (organizationRepository.existsBySlug(DEMO_SLUG)) {
            ...
            try {
                if (patientRepository.count() == 0) {
                    UUID adminUserId = userRepository.findByEmail("admin@democlinic.example")
                            .orElseThrow(() -> new IllegalStateException("Demo clinic admin missing"))
                            .getId();
        ...
        // point and guarantees this row exists.
        ...
        var provisioned = provisioningService.provisionOrganization(
                new OrganizationProvisioningService.ProvisionOrganizationCommand(
                        ...
                        List.of(new OrganizationProvisioningService.AdminInput(
                                ...
                                "Dlamini",
                                "EMP-0001",
                                "admin@democlinic.example",
                                "+27821234501",
                                Gender.FEMALE))),
                operatorId);

        Organization organization = organizationRepository
                .findById(provisioned.organizationId())
                .orElseThrow();
    ...
    private void seedTenantData(UUID adminUserId) {
        Facility mainClinic = facilityService.create(
                "Demo Main Clinic",
                "DEMO-01",
                FacilityType.CLINIC,
                "12 Cradle Street, Johannesburg",
                "+27115550101",
                "Mon-Fri 07:00-17:00");

        Facility satelliteClinic = facilityService.create(
                "Demo Satellite Clinic",
                "DEMO-02",
                FacilityType.CLINIC,
                "44 Baobab Avenue, Soweto",
                "+27115550102",
                "Mon-Fri 08:00-16:00");

        UUID doctorId = createStaffMember(
                mainClinic.getId(),
                "Doctor",
                "Sipho",
                "Mahlangu",
        ...
        createStaffMember(
                ...
                "EMP-1005",
                "9204175800087",
                "t.sithole@democlinic.example",
                "+27821234506",
                Gender.MALE,
                LocalDate.of(1992, 4, 17),
                "Administration",
                "Admin Officer",
                adminUserId);

        createStaffMember(
                satelliteClinic.getId(),
                "Facility Manager",
                "Lindiwe",
                "Zulu",
                "EMP-1006",
                "8309305800088",
                "l.zulu@democlinic.example",
                "+27821234507",
                Gender.FEMALE,
    ...
    private void seedPatientData(UUID adminUserId) {
        ...
        // which reads the acting role from SecurityContextHolder. There is no
        // real HTTP request/JWT here, so a scoped fake principal stands in for
        // an ORG_ADMIN token during development seeding.
        ...
        try {
            ...
            registerPatient(
                    ...
                    1002,
                    "5 Nelson Mandela Drive, Johannesburg",
                    "+27831112202",
                    null,
                    null,
                    adminUserId);
            ...
            registerPatient(
                    "Kagiso",
                    "Modise",
                    LocalDate.of(1960, 11, 22),
                    6003,
                    "22 Freedom Square, Soweto",
                    "+27831112203",
                    "Bonitas",
                    "BN-441290",
                    adminUserId);
            ...
            registerPatient(
                    "Sibusiso",
                    "Radebe",
                    LocalDate.of(2001, 4, 30),
                    6005,
                    "31 Chris Hani Road, Johannesburg",
                    "+27831112205",
                    "Momentum Health",
                    "MH-556723",
                    adminUserId);
    ...
    private UUID createStaffMember(
            UUID facilityId,
            String roleName,
            String firstName,
            String lastName,
        ...
        Role role = roleRepository.findByName(roleName)
                .orElseThrow(() -> new IllegalStateException(
                        roleName + " role missing — seed migration didn't run"));

        User user = staffService.createStaff(
                new StaffService.CreateStaffCommand(
                        firstName,
                        lastName,
                        employeeNumber,
                        idNumber,
    ...
    private void registerPatient(
        ...
        patientService.register(
                new PatientService.RegisterPatientCommand(
                        firstName,
                        lastName,
                        idNumber,
                        address,
    ...
    // (<5000 female, >=5000 male) for the parsed-back gender to match.
    private static String southAfricanIdNumber(
            LocalDate dateOfBirth,
            int genderSequence) {
