...
public class StaffService {
    ...
    // profile, role, and clinic field themselves. The account receives a
    // server-generated password hash that is never disclosed; the real
    // credential is chosen through the one-time setup link emailed below.
    // noRollbackFor is load-bearing, same reasoning as
    ...
    public User createStaff(CreateStaffCommand cmd, UUID creatingAdminId) {
        ...
        User user = new User(cmd.employeeNumber(), cmd.email(), cmd.firstName(), cmd.lastName(),
            cmd.contactNumber(), passwordEncoder.encode(temporaryPasswordGenerator.generate()), cmd.facilityId(),
            cmd.gender());
        ...
        String setupCode = passwordResetService.issueSetupToken(user.getId());
        organizationRepository.findBySchemaName(TenantContext.getCurrentTenant())
                ...
                user.getEmployeeNumber(), setupCode));
