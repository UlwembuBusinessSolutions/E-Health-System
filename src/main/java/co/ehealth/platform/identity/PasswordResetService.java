...
public class PasswordResetService {
    ...
    public String issueSetupToken(java.util.UUID userId) {
        User user = userRepository.findById(userId).orElseThrow(() -> new IllegalArgumentException("Unknown staff member"));
        String code = String.format("%06d", RANDOM.nextInt(1_000_000));
        tokenRepository.invalidateOutstandingTokens(user.getId(), clock.instant());
        tokenRepository.save(new PasswordResetToken(user.getId(), passwordEncoder.encode(code),
                clock.instant().plus(CODE_TTL), "staff-creation"));
        return code;
    }
