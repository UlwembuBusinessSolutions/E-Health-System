...
public record QueueTokenResponse(UUID id, UUID visitId, UUID facilityId, UUID stationId, int tokenNumber, TokenPriority priority,
                                  TokenStatus status, boolean manual, Instant issuedAt, Instant calledAt) {
    static QueueTokenResponse from(QueueToken t) {
        return new QueueTokenResponse(t.getId(), t.getVisitId(), t.getFacilityId(), t.getStationId(), t.getTokenNumber(),
