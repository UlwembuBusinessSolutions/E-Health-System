...
// Clears the automatic lockout from five failed login attempts — distinct
// from enable/disable above, which toggle a deliberate ACTIVE/DISABLED
// decision rather than this automatic LOCKED state.
export async function unlockOperator(operatorId: string): Promise<void> {
  await apiClient.post<void>(`/platform/operators/${operatorId}/unlock`, undefined, { headers: authHeaders() });
}

// Permanent removal, unlike setOperatorEnabled(false) above — refuses
// (409) to delete the last active operator (same guard as disable), and
// separately refuses (409) to delete any operator with existing platform
// audit history (PlatformOperatorService.delete()'s own why-note).
export async function deleteOperator(operatorId: string): Promise<void> {
  await apiClient.delete<void>(`/platform/operators/${operatorId}`, { headers: authHeaders() });
}
