...
// from setStaffEnabled above, which toggles a deliberate ACTIVE/DISABLED
// decision rather than this automatic LOCKED state.
export async function unlockStaffAccount(staffId: string): Promise<void> {
  await apiClient.post<void>(`/api/v1/admin/staff/${staffId}/unlock`, undefined, { headers: tenantAuthHeaders() });
}
