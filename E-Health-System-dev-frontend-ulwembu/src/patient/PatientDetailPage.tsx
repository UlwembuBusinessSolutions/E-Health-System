    ...
    <div>
      ...
      {patientQuery.isLoading ? (
        ...
        <>
          ...
          <Card className="p-6">
            <h2 className="mb-4 text-[14.5px] font-semibold text-text-primary">Demographics</h2>
          ...
          <Card className="mt-6 p-6">
            <h2 className="mb-4 text-[14.5px] font-semibold text-text-primary">Next of kin or guardian</h2>
            {patient.nextOfKin.length === 0 ? (
              <p className="text-[13.5px] text-text-secondary">No emergency contact recorded.</p>
            ) : (
              <div className="grid grid-cols-1 gap-5 sm:grid-cols-2 lg:grid-cols-3">
                {patient.nextOfKin.map((contact, index) => (
                  <div key={`${contact.contactNumber}-${index}`}>
                    <p className="text-[11px] font-medium uppercase tracking-wide text-text-secondary">
                      Contact {index + 1}
                    </p>
                    <p className="mt-1 text-[13.5px] font-medium text-text-primary">{contact.name}</p>
                    <p className="text-[13px] text-text-secondary">{contact.relationship}</p>
                    <p className="mt-1 text-[13.5px] text-text-primary">{contact.contactNumber}</p>
                  </div>
                ))}
              </div>
            )}
